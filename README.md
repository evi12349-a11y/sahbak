...
בכתובת `/exams` זמין מרחב תרגול רספונסיבי לקורס יחיד: בחירת מבחן לפי שנה,
ניווט בין שאלות, הצגת פתרון, וסימולציית מבחן מלאה עם טיימר וציון. הממשק
משתמש כרגע בנתוני הדגמה מסומנים (`MOCK DATA`) בקובץ `exams.html`, משום שקבצי
ה-PDF הסרוקים עדיין אינם מחוברים לצינור חילוץ. מבנה הנתונים הוא
`exam -> questions -> options/answer/solution`, כך שניתן להחליף את המערך בנתוני
parser בהמשך בלי לשנות את ממשק התרגול.
